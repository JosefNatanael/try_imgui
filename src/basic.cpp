...
int main(int, char**) {
...
#endif
    {
        ...
        if (appState.show_demo_window) ImGui::ShowDemoWindow(&appState.show_demo_window);

        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
        op::renderBasicWidget(appState, io);

        // 3. Show another simple window.
        if (appState.show_another_window) {
            ImGui::Begin("Another Window",
                         &appState.show_another_window);  // Pass a pointer to our bool variable (the window will have a
                                                          // closing button that will clear the bool when clicked)
            ImGui::Text("Hello from another window!");
            if (ImGui::Button("Close Me")) appState.show_another_window = false;
            ImGui::End();
        }
